...
def find_transaction_signatures(reference: PublicKey, before: Optional[TransactionSignature] = None, 
        limit: Optional[int] = 1000, until: Optional[TransactionSignature] = None, commitment: Commitment = None):
    try: 
        # Solana blockchain api
        client = Client(settings.SOLANA_API_URL)
        # Retrieve the new signatures
        signatures = client.get_signatures_for_address(reference, before, until, limit, commitment).get('result')
    except: 
