...
def NFTMB_PaymentSystem1(my_address, puppy_price):
    ...
    def payment_and_metadata(database_table=nftmysterybox):
        ...
        tuple_result = metadataNFT.main_NFTMetadata(NFT_Types.puppy[1], NFT_Types.puppy[0])
    ...
    # If the thread is still running after 9 seconds (timeout), terminate the thread
    if thread.is_alive():
        return "timeout"
    if type(result) != tuple:
        return result
