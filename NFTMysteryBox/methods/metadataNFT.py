...
from CoreMysteryBox import privatedatas
...
def google_image_search(words: Tuple[str, ...], word_type):
    ...
    for i in range(10):
        try:
            ...
            params = {
                "key": privatedatas.gcs_api_key,
                "cx": privatedatas.gcs_cx,
                "q": word + " " + word_type,
                "searchType": "image",
                "num": 1,
            }
...
def NFT_metadata(name: str, symbol: str, description: str, img_link: str, webshop_link: str):
    try:
        ...
        r = requests.put(
            ...
            headers = {
                'Authorization': f'Token {privatedatas.github_api_token}'
            },
            json = {
        ...
        uri = r.json()["content"]["download_url"]
        return uri
    except:
...
def main_NFTMetadata(words, word_type):
    ...
    uri = NFT_metadata(
        name=key_word, 
        symbol=word_type, 
        description="Puppy NFT from the revolutionary Solana Mystery Box shop!",
        img_link=image_link,
